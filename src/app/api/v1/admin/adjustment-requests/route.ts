...
import { listPendingAdjustRequests } from "@/server/teacher/adjust-requests";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const requests = await listPendingAdjustRequests();
    return jsonNoStore({ requests });
  } catch (err) {
    console.error("adjustment-requests list failed", err);
